...
- **Mention table in chat** (or **Mention text in chat** / **Mention canvas in chat**, named for the block type) and **Explain with Ari**, which hand the block to Ari.
- **Duplicate block**.
- **Move to page**, with a list of Pages you can edit plus **New page**.
- **Delete block**.

If the block is the home of a Variable or Dimension that other content reads, cfo.ai asks before deleting it. A dialog titled **Delete variable or dimension?** counts what would break, such as **Deleting Revenue will break 2 formulas, 1 block, 1 canvas block.** A Canvas block counts when one of its datasets reads the Variable. Choose **Cancel** to keep everything, or **Delete anyway** to delete the block and the Variable together. Deleting a Page whose tables are the home of such a Variable asks the same question.

...
## Download a file you attached

Each attached file shows as a tile in the conversation with its name and file type. Select the tile's **Download** button to get the file back, for example to hand a colleague the workbook a decision was based on. A file Ari produces for you appears as a card that downloads when you select it.
...
- If a conversation grows too large for Ari to read in one request, the chat says **This request is too large for Ari to continue. Start a new chat with a shorter request or fewer attachments.** Start a new chat and attach only the files the new request needs. See [When a reply is interrupted](/ari/during-a-conversation#when-a-reply-is-interrupted).

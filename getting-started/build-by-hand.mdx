...
- **Chat pane header**. When the Ari chat is docked beside the Model, its header has a **Collapse chat pane** button showing inward-pointing arrows. The floating popout shows the matching outward-pointing **Expand chat** button.
...
<Steps>
  ...
  <Step title="Or upload a CSV">
    Select **Upload** in the **File-based imports** section. You can drop several files at once. Each file becomes its own connection named after the file, and each file can be up to 50 MB. Uploading is not restricted, but cfo.ai processes about five CSV imports per workspace at a time, so a large batch queues and finishes in turn.
  </Step>

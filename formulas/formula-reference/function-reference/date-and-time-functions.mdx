...
<Warning>
  **Naming collision with Excel:** cfo.ai's `offset` shifts a time period. Excel's `OFFSET` returns a cell range shifted by rows and columns — an unrelated operation. If you or an agent are used to Excel, don't assume `offset` behaves the same way here.
</Warning>

<Warning>
  **🚧 Not yet available:** `datedif`, `daysin`, `date`, `edate`, and `eomonth` below are designed but not yet implemented. Using any of them in a formula today will fail with an "Unknown function" error. `offset` above is the only function on this page that works today. Of the unimplemented ones, a date-difference function (name still undecided — may end up as `datedif` or `age`) is part of the current top build priority, and `daysin` is fourth in line; `date`, `edate`, and `eomonth` don't have a confirmed timeline yet.
</Warning>

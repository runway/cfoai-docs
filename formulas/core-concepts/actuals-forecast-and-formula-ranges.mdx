...
## Imported Variables in Forecast

A Variable imported from a connected source reads its source column only through Last close. Forecast periods read zero unless that Variable has its own Forecast rule, so a formula that reads an imported Variable in a forecast month reads zero there. When this happens, the write receipt says so and names the Forecast rule that would read the source rows dated in each forecast period.

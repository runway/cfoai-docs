...
A Variable has a default way to roll underlying values into a model cell. Variables normally use `sum`. Dimensions normally use `first` because they describe a group rather than a numeric total.
...
This selects Revenue across all Department values. If no aggregation function is written, a multi-value Variable lookup uses `sum`.

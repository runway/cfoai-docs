...
Use the saved table's existing Scenario or time comparison when it already answers the question. For a top-items question, use the `rank` read on one saved Table Block.

...
On a Table Block that shows **Gross Profit**, hover its row label and select the **fx** icon (**Edit variable formula**), or right-click the row label and choose **Edit formula for Gross Profit**. The editor's title shows **Gross Profit** and **Any Segment**, because a row-level formula is the Variable's default. The formula belongs to Gross Profit because that is the value it calculates.

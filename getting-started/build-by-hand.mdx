...
| **Pin column** or **Unpin column** | Keep a column beside the row names while the other columns scroll. Right-click the column's header; a Date column and the columns grouped under it offer no **Pin column**. A solid line marks the last pinned column, and pins are saved with the table, so they hold after a reload and for everyone who opens it. |
...
- **Pin a column** by dragging its header left into the pinned area beside the row names, or right-click the header and choose **Pin column**. Pins are saved with the table.

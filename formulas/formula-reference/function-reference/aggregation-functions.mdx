...
Return the first or last value in the selected series order, or null when the series is empty. Over an imported record's rows, `first` and `last` order the rows by the source's preferred time column, so `last` is the row with the latest date there; rows on one date keep their source order.

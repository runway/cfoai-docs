...
The Page header shows an access badge (**Private**, **Shared**, or **Workspace**) and a **Share** button. Select **Share** to invite people by name or email as **Can view** or **Can edit**, choose between **Everyone at \[your workspace\]** and **Restricted** under **General access**, and select **Create link** under **External access** for a read-only external link with an optional **Require password** setting. **Copy link** copies the ordinary signed-in address without changing anyone's access. See [Share a Page and choose its Scenario](/dashboards/share-pages-and-scenarios).

## Download a Page as a spreadsheet

Select **Download spreadsheet** in the Page header. Each visible Table Block becomes a worksheet, formulas are exported as real cross-sheet formulas, and any inputs the tables depend on land on a **Model inputs** sheet. The export runs in the background; keep working and download the file when it is ready.
...
To invite a teammate to a Page, use the Page's **Share** control and enter their name or email. Ari can also invite a workspace administrator; see [Workspace and team settings](/workspace-settings/team-and-workspace).

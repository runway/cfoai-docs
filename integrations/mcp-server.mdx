...
If your account belongs to one workspace, the connection can use it automatically. If your account belongs to several workspaces, ask the agent to call `list_orgs` and select the correct workspace by its ID or slug before reading or changing data.

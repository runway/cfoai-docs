...
<Tip>
Press **⌘ ?** (**Ctrl ?** on Windows and Linux) at any time to open the Help center, which includes short interactive walkthroughs and the keyboard shortcut list. Under **Additional resources**, **Ask a human** opens a form that sends your message straight to the cfo.ai team, with up to three PNG, JPG, or WebP screenshots of 5 MB or less. Press **⌘ Enter** (**Ctrl Enter**) to send it.
</Tip>
...
| **Pin column** or **Unpin column** | Keep a column beside the row names while the other columns scroll. Right-click the column's header; a Date column, and the columns grouped under it, offers no **Pin column**. A solid line marks the last pinned column, and pins are saved with the table, so they hold after a reload and for everyone who opens it. |
...
Hover a connection for its actions: **Reconnect** or **Resume** when setup is incomplete or the source revoked access, **Update file** to replace a CSV in place, **Rename**, and **Remove**. An MCP server or API connection offers **Manage** and **Disconnect** instead. For details on each source, see [Connect and sync your data](/integrations/connect-and-sync-data).
